...
def strip_22_early_out(text, handle_line, label):
    """Remove the 2.2 user-pointer early-out that sits before user_path_at."""
    pat = re.compile(
...
# ---- fs/stat.c vfs_fstatat / newfstatat ----
...
#endif'''
if 'ksu_handle_stat(&dfd, &fname, &flag)' in stat_t and \
   'filename_lookup(dfd, fname, lookup_flags, &path, NULL)' in stat_t:
    ...
    print('fs/stat.c: vfs_fstatat user_path_at -> getname_flags + filename_lookup + putname', flush=True)
else:
    old_call = 'ksu_handle_stat(&dfd, &filename, &flag);'
    new_call = '''{
...
\t\t}'''
    if old_call in stat_t:
        ...
        print('fs/stat.c: newfstatat handle rewritten to filename**', flush=True)
    else:
...
if 'ksu_handle_stat(&dfd, &fname, &flag)' not in stat_t:
    fail('fs/stat.c missing ksu_handle_stat(&dfd, &fname, &flag)')
