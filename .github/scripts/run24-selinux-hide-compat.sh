...
echo '===== Do not force -DKSU_COMPAT_USE_STATIC_KEY; infra already defines it on 4.14 ====='

echo '===== Ungate KSU_FEATURE_SELINUX_HIDE in pinned SukiSU UAPI for 4.14 ====='
python3 - <<'PY'
from pathlib import Path

candidates = [
    Path('KernelSU/kernel/include/uapi/feature.h'),
    Path('KernelSU/kernel/uapi/feature.h'),
    Path('KernelSU/uapi/feature.h'),
]
p = next((c for c in candidates if c.exists()), None)
if p is None:
    raise SystemExit('SukiSU uapi/feature.h not found')
s = p.read_text()
old = '''#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
    KSU_FEATURE_SELINUX_HIDE = 4,
#endif
'''
new = '''    KSU_FEATURE_SELINUX_HIDE = 4,
'''
if 'KSU_FEATURE_SELINUX_HIDE = 4' not in s:
    raise SystemExit(f'{p} missing KSU_FEATURE_SELINUX_HIDE')
if old in s:
    s = s.replace(old, new, 1)
    p.write_text(s)
    print(f'[PASS] unguarded KSU_FEATURE_SELINUX_HIDE in {p}')
elif '#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)' in s and 'KSU_FEATURE_SELINUX_HIDE' in s:
    raise SystemExit(f'{p} still gates KSU_FEATURE_SELINUX_HIDE behind 5.10')
else:
    print(f'[PASS] {p} already exposes KSU_FEATURE_SELINUX_HIDE on 4.14')
PY
...
if '#ifndef KSU_FEATURE_SELINUX_HIDE' not in s:
    s = s.replace(
...
grep -Fq 'KSU_FEATURE_SELINUX_HIDE = 4' KernelSU/kernel/include/uapi/feature.h
! grep -Fq '#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)' KernelSU/kernel/include/uapi/feature.h
! grep -Fq 'ccflags-y += -DKSU_COMPAT_USE_STATIC_KEY' KernelSU/kernel/Makefile

{
  ...
  echo 'feature_id_ungated=KSU_FEATURE_SELINUX_HIDE=4'
