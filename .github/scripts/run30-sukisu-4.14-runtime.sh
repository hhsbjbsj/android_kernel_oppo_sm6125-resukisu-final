...
echo 'then ioctl(KSU_IOCTL_GET_INFO). Handshake MUST run before capable().'
echo 'SUSFS builds compile ksu_handle_sys_reboot from supercall/dispatch.c.'
...
def patch_dispatch_magic2(t, path):
    if 'install fd for official manager (sync 4.14 sys_reboot)' in t and 'if (magic2 == KSU_INSTALL_MAGIC2)' in t:
        print('%s: MAGIC2 already sync' % path, flush=True)
        notes.append('dispatch_magic2=already_sync')
        return t
    new_t, n = re.subn(
        r'[ \t]*if\s*\(\s*magic2\s*==\s*KSU_INSTALL_MAGIC2\s*\)\s*\n'
        r'[ \t]*return\s+ksu_supercall_reboot_handler\s*\(\s*arg\s*\)\s*;',
        MAGIC2_SYNC,
        t,
        count=1,
    )
    if n:
        notes.append('dispatch_magic2=sync')
        print('%s: MAGIC2 now sync-installs manager fd' % path, flush=True)
        return new_t
    print('%s: WARN MAGIC2 call not rewritten' % path, flush=True)
    notes.append('dispatch_magic2=unchanged')
    return t
...
for rel in [
    ...
    if p.name == 'dispatch.c':
        t = patch_dispatch_magic2(t, p)
    if t != orig:
        p.write_text(t)
...
if reboot.exists():
    ...
    t = re.sub(
        ...
        r'\textern int ksu_handle_sys_reboot[^\n]+\n'
        r'(?:\t[^\n]+\n)?'
        r'#endif\n',
        '\n',
        t,
    ...
    t = re.sub(
        r'\n#if defined\(CONFIG_KSU\)\n'
        r'\tif \(!ksu_handle_sys_reboot\(magic1, magic2, cmd, &arg\)\)\n'
        r'\t\treturn 0;\n'
        r'#endif\n',
        '\n',
        t,
    ...
    t = re.sub(
        r'\n[ \t]*ksu_handle_sys_reboot\s*\(\s*magic1\s*,\s*magic2\s*,\s*cmd\s*,\s*&arg\s*\)\s*;\n',
        '\n',
        t,
    )
    needles = [
    ...
    insert_body = (
        '#if defined(CONFIG_KSU)\n'
        '\t/* before capable()/LINUX_REBOOT_MAGIC: official manager is unprivileged */\n'
    ...
    for needle in needles:
        if needle in t:
            t = t.replace(needle, needle + insert_body, 1)
            placed = True
            notes.append('reboot_c=before_capable')
            print('kernel/reboot.c: handshake at sys_reboot entry before capable()', flush=True)
            break
    if not placed:
        print('kernel/reboot.c: WARN could not place pre-capable handshake', flush=True)
...
dc=""
if [[ -f KernelSU/kernel/supercall/dispatch.c ]]; then
  dc=KernelSU/kernel/supercall/dispatch.c
elif [[ -f drivers/kernelsu/supercall/dispatch.c ]]; then
  dc=drivers/kernelsu/supercall/dispatch.c
fi
if [[ -n "$dc" ]]; then
  grep -Fq 'int ksu_handle_sys_reboot' "$dc"
  grep -Fq 'install fd for official manager (sync 4.14 sys_reboot)' "$dc"
fi
grep -Fq 'ksu_handle_sys_reboot' kernel/reboot.c
grep -Fq 'if (!ksu_handle_sys_reboot(magic1, magic2, cmd, &arg))' kernel/reboot.c
grep -Fq 'before capable()/LINUX_REBOOT_MAGIC' kernel/reboot.c
...

{
  ...
  echo 'official_handshake=dispatch_magic2_sync_fd'
  echo 'reboot_return=0_on_magic_before_capable'
...
echo '[PASS] official SukiSU manager handshake is dispatch.c MAGIC2 sync fd + version 40900/v4.2.0'
