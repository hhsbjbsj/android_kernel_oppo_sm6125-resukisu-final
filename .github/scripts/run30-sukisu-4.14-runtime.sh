...
echo 'supercall.c only has the #else copy. Do not emit a second definition.'
...
if [[ -n "$sc" ]]; then
  grep -Fq 'int ksu_handle_sys_reboot' "$sc" || true
...
if [[ -n "$dc" ]]; then
  ...
  ! grep -Fq 'return ksu_supercall_reboot_handler(arg);' "$dc"
fi
