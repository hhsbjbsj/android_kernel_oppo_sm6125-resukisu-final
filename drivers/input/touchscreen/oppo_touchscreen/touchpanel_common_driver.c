...
#include "touchpanel_common_375.h"
#include "util_interface/touch_interfaces.h"
...
int register_common_touch_device(struct touchpanel_data *pdata)
{
    ...
    if ((ts->boot_mode == META_BOOT || ts->boot_mode == FACTORY_BOOT))
#else
    if ((ts->boot_mode == MSM_BOOT_MODE__FACTORY &&(!strstr(saved_command_line, "dsi_nt36672a_tm_vid_display")))|| ts->boot_mode == MSM_BOOT_MODE__RF || ts->boot_mode == MSM_BOOT_MODE__WLAN)
#endif
    {
        ts->ts_ops->ftm_process(ts->chip_data);
        ret = -EFTM;
