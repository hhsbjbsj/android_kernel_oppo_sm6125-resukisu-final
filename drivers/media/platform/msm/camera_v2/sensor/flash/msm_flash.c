...
#ifdef VENDOR_EDIT
/* Haiyuan.Sun@CAMERA_HQ_ODM add for torch status to modify charg  */
#include <linux/delay.h>

extern int set_volt(bool isTorchOn);
//EXPORT_SYMBOL(set_volt);
extern bool get_volt( void );
    ...
    else if (strcmp(vendor_flash_ctrl->flash_name, "pmic") == 0)
    {
        ...
        switch (flash_mode)
        {
            case 0:
                ...
                #ifdef VENDOR_EDIT
                set_volt(FALSE);
                //mdelay(100);//  wait for charge thread
               pr_err("Torch turn off \n");
                #endif
            ...
            case 1:
                ...
                /*20190513, Setting torch current to 55mA*/
                   ...
                   set_volt(TRUE);
                   mdelay(100);    //  wait for charge thread
                   pr_err("Torch  turn on \n");

                } else {
                    flash_data.flash_current[0] = 110; /*110mA*/
                    flash_data.flash_current[1] = 110; /*110mA*/
                }
                #endif
