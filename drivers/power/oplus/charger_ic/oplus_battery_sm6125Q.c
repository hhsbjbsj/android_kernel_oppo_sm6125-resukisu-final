...
#ifdef VENDOR_EDIT
/* wangdengwen add for QC torch status change */
...
void smblib_hvdcp_detect_enable(struct smb_charger *chg, bool enable)
{
	...
	u8 mask;
	#ifndef VENDOR_EDIT
	/* wangdengwen  changed PD adapter compatible at 6125Q */
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if(chip->dt.vsysmin_support){
		smblib_read(chg, 0x1183, &reg_before);
		smblib_write(chg, 0x1183, 0x05);
		smblib_read(chg, 0x1183, &reg_after);
...
#endif
#endif
/*wangdengwen move out the CONFIG_HIGH_TEMP_VERSION for clear ADC1 pulse per second*/
#ifdef VENDOR_EDIT
...
static int opluschg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	smblib_get_iio_channel(chg, " usb_temp_v_l", &chg->iio.usb_temp_chan1);
	smblib_get_iio_channel(chg, " usb_temp_v_r", &chg->iio.usb_temp_chan1);
...
static int smb5_probe(struct platform_device *pdev)
{
	...
	int rc = 0;
#ifdef VENDOR_EDIT
	/* wangdengwen  for QC2.0 adapter compatibility*/
	int flag;
	union power_supply_propval ret = {0, };
#endif
