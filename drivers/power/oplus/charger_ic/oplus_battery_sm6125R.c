...
EXPORT_SYMBOL(set_volt);

bool get_volt( void );
EXPORT_SYMBOL(get_volt);
#endif
...
#ifdef OPLUS_FEATURE_CHG_BASIC
		printk(KERN_ERR "smblib_get_prop_usb_online val=%d\n",val->intval);
#endif
		return rc;
	}
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if(g_oplus_chip){
			do_gettimeofday(&(g_oplus_chip->stChargerDetect.stVbusOnTime));
			if(g_oplus_chip->stChargerDetect.isForceType){
				smblib_err(chg, "WDWCHG STOP HVDCP&APSD rc=%d\n", rc);
...
#if (SHIPPING_API_LEVEL == 29)
		if(g_oplus_chip){
			do_gettimeofday(&(g_oplus_chip->stChargerDetect.stVbusOffTime));
			smblib_err(chg, "WDWCHG VBUS OFF time=%d\n",(g_oplus_chip->stChargerDetect.stVbusOffTime.tv_sec - g_oplus_chip->stChargerDetect.stVbusOnTime.tv_sec));
			if((g_oplus_chip->stChargerDetect.stVbusOffTime.tv_sec - g_oplus_chip->stChargerDetect.stVbusOnTime.tv_sec) > 3){
				smblib_err(chg, "WDWCHG VBUS OFF reset\n");
				g_oplus_chip->stChargerDetect.iQcDetectCount   = 0;
				g_oplus_chip->stChargerDetect.iDcpDetectCount  = 0;
				g_oplus_chip->stChargerDetect.iUsbPlugIrqCount = 0;
			}
			if(g_oplus_chip->stChargerDetect.isResetType){
				smblib_err(chg, "WDWCHG RERUN HVDCP&APSD rc=%d, forcetype=%d\n", rc , g_oplus_chip->stChargerDetect.isForceType);
				smblib_hvdcp_detect_enable(chg, true);
			}
		}
#endif
...
irqreturn_t usb_plugin_irq_handler(int irq, void *data)
{
	...
	struct oplus_chg_chip *chip = g_oplus_chip;
	if (NULL == chip) {
		smblib_err(chg, "chip is NULL ,retrun IRQ!!!");
		return IRQ_HANDLED;
	}

	if(chip){
		chip->stChargerDetect.iUsbPlugIrqCount++;
	}
...
#if (SHIPPING_API_LEVEL == 29)
	if( POWER_SUPPLY_TYPE_USB_DCP == apsd_result->pst ){
		chip->stChargerDetect.iDcpDetectCount += 1;
		printk(KERN_ERR "!!!IRQ: apsd-done rising; %s dcp detected count=%d\n",
					apsd_result->name, chip->stChargerDetect.iDcpDetectCount);
	}
	else if(POWER_SUPPLY_TYPE_USB_HVDCP == apsd_result->pst){
		chip->stChargerDetect.iQcDetectCount += 1;
		printk(KERN_ERR "!!!IRQ: apsd-done rising; %s qc detected count=%d\n",
					apsd_result->name, chip->stChargerDetect.iQcDetectCount);
	}
	else{
		chip->stChargerDetect.iDcpDetectCount = 0;
		chip->stChargerDetect.iQcDetectCount  = 0;
		if( chip->stChargerDetect.isForceType )
		{
			chip->stChargerDetect.isForceType      = false;
			chip->stChargerDetect.isTimeCountStart = false;
			smblib_hvdcp_detect_enable(chg, true);
			chip->stChargerDetect.isRecoverUsbIn   = true;
		}
		printk(KERN_ERR "!!!IRQ: apsd-done rising; %s count_all=0\n",
					apsd_result->name);
	}
#endif
...
static void typec_src_insertion(struct smb_charger *chg)
{
	...
	if (!chg->ok_to_pd && g_oplus_chip && !g_oplus_chip->stChargerDetect.isForceType)
		smblib_hvdcp_detect_enable(chg, true);
}
...
#define PMI632_MAX_ICL_UA	3200000
...
static int smb5_parse_dt(struct smb5 *chip)
{
	...
	ti27541_enable = of_property_read_bool(node, "qcom,ti27541-enable");
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		val->intval = oplus_chg_get_charger_subtype();
		break;
#endif
	default:
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if(chip->dt.vsysmin_support){
		...
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read 0x1183 before, rc=%d\n",
					rc);
			return rc;
		}
		rc = smblib_write(chg, 0x1183, 0x05);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't write 0x1183, rc=%d\n",
					rc);
			return rc;
		}
		rc = smblib_read(chg, 0x1183, &reg_after);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't read 0x1183 after, rc=%d\n",
					rc);
			return rc;
		}
	...
	vote(chg->fcc_votable,
		BATT_PROFILE_VOTER, chip->dt.batt_profile_fcc_ua > 0,
		chip->dt.batt_profile_fcc_ua);
	vote(chg->fv_votable,
		BATT_PROFILE_VOTER, chg->batt_profile_fv_uv > 0,
		chg->batt_profile_fv_uv);

	vote(chg->usb_icl_votable, USB_PSY_VOTER, true, USBIN_100MA);
...
static void opluschg_debug_func(struct work_struct *w)
{
	...
	if (charge_debug_mask & DEBUG_MASK_KEY_FLAG) {
		rc = smblib_read(chg, TYPE_C_MODE_CFG_REG, &stat[4]);
		if (rc < 0) {
			chg_err("Couldn't read TYPE_C_MODE_CFG_REG rc=%d\n", rc);
			return;
		}
		rc = power_supply_get_property(chg->usb_psy, POWER_SUPPLY_PROP_TYPEC_MODE, &val);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get TYPEC_MODE, rc=%d\n",rc);
			return;
		}
	...
	for (j = 0; j < 7; j ++) {
		...
		for (i = 0; i < 16; i ++) {
			for (k = 0; k < 16; k ++) {
				rc = smblib_read(chg, reg_base[j] + (16*i + k), &stat[k]);
				if (rc < 0) {
					chg_err("Couldn't read reg_base[] rc=%d\n", rc);
					return;
				}
			}
...
#if (SHIPPING_API_LEVEL == 28)
/* shanliangliang BSP.CHG.Basic 2020/03/23 HVDCP charger Brandon isn't support */
	if ((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP)
		|| (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3)) {
		type = POWER_SUPPLY_TYPE_USB_DCP;
		goto get_type_done;
...
static void opluschg_usbtemp_dischg(void)
{
	...
	//add by wangdengwen for bug(2661692) of 19375
	//-------------------------start-----------------------
	rc = smblib_write(chg, TYPE_C_MODE_CFG_REG, 1);
	if (rc < 0)
	{
		chg_err("fail to write disable cc at pmic register, rc = %d\n", rc);
	}
	usleep_range(10000,11000);
	//-------------------------end-------------------------
...
int set_volt(bool isTorchOn)
{
	chg_debug("isTorchOn: %d ischargerok:%d\n", isTorchOn, g_oplus_chip?1:0);
...
static int opluschg_usbtemp_monitor_main(void *data)
{
	...
	while (!kthread_should_stop()) {
		if (opluschg_usbtemp_condition() == true) {
			...
			resistance1 = voltage_convert_resistance(voltage1, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_100KOHM);
			resistance2 = voltage_convert_resistance(voltage2, PULL_UP_VOLTAGE_1800MV, PULL_UP_RESISTANCE_100KOHM);
			...
			if(isQcTorchEn && !isVbusSet){
				vBusSetEnd = false;
				opluschg_set_vbus(VBUS_LIMIT_5V);
				isVbusSet = true;
				vBusSetEnd = true;
			}
			else if(!isQcTorchEn && isVbusSet){
				vBusSetEnd = false;
				opluschg_set_vbus(g_oplus_chip->limits.qc_input_volt_charger_v);
				isVbusSet = false;
				vBusSetEnd = true;
			}
#endif
...
static int opluschg_set_vbus(int vbus)
{
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if(vbus == VBUS_LIMIT_5V || isQcTorchEn)
#else
	...
	if (rc < 0) {
		...
		pr_debug("opluschg_set_vbus limit = %d",vbus);
	}
...
{
	int rc = 0, i = 0, chg_vol = 0, aicl_point = 0;
	static int pre_current = 0;
	u8 stat = 0;
	struct oplus_chg_chip *chip = g_oplus_chip;
...
#else
	if ((chip->pmic_spmi.smb5_chip->chg.pre_current_ma == current_ma) && (!chip->stChargerDetect.isRecoverUsbIn)) {
#endif
		return rc;
	} else {
	...
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, BOOST_BACK_VOTER) == 0
		&& get_effective_result(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <= USBIN_25MA) {
		chg_debug("use 500 here 1\n");
		goto aicl_boost_back;
	}
	chg_vol = opluschg_read_adc(USBIN_VOLTAGE);
	if (get_client_vote(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable, BOOST_BACK_VOTER) == 0
		&& get_effective_result(chip->pmic_spmi.smb5_chip->chg.usb_icl_votable) <= USBIN_25MA) {
		chg_debug("use 500 here 2\n");
		goto aicl_boost_back;
	}
	if (chg_vol < aicl_point) {
		chg_debug("use 500 here 3\n");
		chip->pmic_spmi.smb5_chip->chg.pre_current_ma = usb_icl[i];
		goto aicl_end;
...
static int opluschg_get_ibus(void)
{
	...
	if (rc < 0) {
		smblib_err(chg, "Couldn't get INPUT_CURRENT_NOW, rc=%d\n",rc);
		return rc;
	}
...
static int opluschg_get_dyna_aicl_result(void)
{
	...
	if (usb_psy) {
		...
		if (rc < 0) {
			chg_err("Couldn't get INPUT_CURRENT_SETTLED rc=%d\n", rc);
			return rc;
		}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if(g_oplus_chip->charger_exist && g_oplus_chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP){
		ret.intval = 1;
		flag = power_supply_set_property(g_oplus_chip->batt_psy, POWER_SUPPLY_PROP_INPUT_SUSPEND, &ret);
		if(flag < 0){
			pr_err("Unable to suspend!");
		}else{
			g_oplus_chip->stChargerDetect.iQcDetectCount   = 0;
			g_oplus_chip->stChargerDetect.iDcpDetectCount  = 0;
			g_oplus_chip->stChargerDetect.iUsbPlugIrqCount = 0;
		...
		flag = power_supply_set_property(g_oplus_chip->batt_psy, POWER_SUPPLY_PROP_INPUT_SUSPEND, &ret);
		if(flag < 0)
			pr_err("Unable to reset suspend!");
	}
#endif
