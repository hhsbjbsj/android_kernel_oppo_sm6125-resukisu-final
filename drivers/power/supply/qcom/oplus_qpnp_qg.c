...
#include <linux/rtc.h>
#include "../../../../kernel/msm-4.14/drivers/power/oplus/oplus_gauge.h"
...
/*Voltage to temperature table for 30k pull up for bat_therm*/
static const struct asic_vadc_map_pt adcmap_batt_therm_asic[] = {
	{1631, 	-400},
	{1619, 	-390},
	{1607, 	-380},
	{1594, 	-370},
...
static int oplus_asic_vadc_map_voltage_temp(const struct asic_vadc_map_pt *pts,
...
static int qg_determine_pon_soc(struct qpnp_qg *chip)
{
	...
	if (((chip->dt.shutdown_soc_threshold != -EINVAL) &&
		...
		(!is_between(0, chip->dt.ignore_shutdown_soc_secs,
			(rtc_sec - shutdown[SDAM_TIME_SEC]))))
		goto use_pon_ocv;
...
#define DEFAULT_SHUTDOWN_SOC_SECS	1800 /* 30 mins */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
#define DEFAULT_SHUTDOWN_SOC_DIFF  20
#endif
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	if (rc < 0)
		chip->dt.shutdown_soc_threshold = DEFAULT_SHUTDOWN_SOC_DIFF;
	else
	...
	if (of_property_read_bool(node, "qcom,tcss-enable")) {
		...
		printk("wangchao tcss-enable =  %d temp = %d\n",chip->dt.tcss_enable,temp);
	}
