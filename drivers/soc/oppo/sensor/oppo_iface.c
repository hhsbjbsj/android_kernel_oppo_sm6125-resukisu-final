...
{
	struct oppo_iface_chip *chip = container_of(work, struct oppo_iface_chip,
				       report_work);
...
static enum alarmtimer_restart light_level_timer_func(struct alarm *alrm,
		ktime_t now)
{
	if (!g_the_chip) {
...
static ssize_t oppo_brightness_store(struct device *pdev,
...
static ssize_t oppo_dc_actived_store(struct device *pdev,
				     struct device_attribute *attr,
				     const char *buff, size_t count)
{
	int dc_actived = -1;
	...
	if (sscanf(buff, "%d", &dc_actived) == 1) {
		...
		printk("[oppo_dc_actived_store] dc_actived %d dc_actived_pre %d", dc_actived,
		       dc_actived_pre);
...
static DEVICE_ATTR(brightness,   S_IRUGO | S_IWUSR, oppo_brightness_show,
		   oppo_brightness_store);
static DEVICE_ATTR(dc_actived,   S_IRUGO | S_IWUSR, oppo_dc_actived_show,
		   oppo_dc_actived_store);
